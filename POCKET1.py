################################ Machine Learnning Foundation 1-18,19 ###################################
    ...
    def countW(self, path):
        ...
        for i in range(self.count):
            if numpy.dot(xdat[i, :], w) * ydat[i,:] <= 0:
                ...
                # test whether the new w is a good one by counting it mistakes(train data set)
                numErr = 0
                for j in range(self.count):
                    if numpy.dot(xdat[j, :], w) * ydat[j,:] <= 0:
                        numErr += 1

                if numErr < num_Best:    # if new w makes fewer mistake, replace w by the new one
                    num_Best = numErr
#18                    w_best = copy.deepcopy(w)
